...
class DispatchOrder extends StatefulWidget {
  ...
  const DispatchOrder({Key? key, required this.userModel, required this.companyModel}) : super(key: key);

  @override
  State<DispatchOrder> createState() => _DispatchOrderState();
}
...
class _DispatchOrderState extends State<DispatchOrder> {
  ...
  getOrder() async{
    await OrderDB(companyId: widget.companyModel.companyId, orderId: "").getDispatchOrder().then((value){
      setState(() {
        order=value;
      });
    });
